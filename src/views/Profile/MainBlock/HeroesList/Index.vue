<template>
  <div class="heroes-list border-top border-secondary mt-5 pt-5">
    <b-table
      ...
      small
    >
      <!-- Customiaza la celda con nombre "name" mediante el componente HeroIco -->
      <template v-slot:cell(name)="data">
        <HeroIco :hero="data.item"/>
      </template>
      <!-- Customiaza la celda con nombre "class" mediante el componente HeroClassLevel -->
      <template v-slot:cell(class)="data">
        <HeroClassLevel :hero="{ class: data.item.classSlug, level: data.item.level}"/>
      </template>
      <!-- Customiaza la celda con nombre "kills" mediante htm simple -->
